 ...
 * `/data/user/0/com.hsucode.app/databases/`。下次启动 Room 打不开库:
 *
 * ```
 ...
 * File /data/user/0/com.hsucode.app/databases/hsucode.db is not readable
 * ```
 *
...
object SelfProtect {
    ...
    /**
     * App 自己的私有数据目录(如 `/data/user/0/com.hsucode.app`)。
    ...
    fun isProtected(path: String): Boolean {
        ...
        val canonicalBase = normalizedPath(base)
        val canonical = normalizedPath(path)
        return LOCKED.any { sub ->
            val locked = "$canonicalBase/$sub"
            canonical == locked || canonical.startsWith("$locked/")
        }
    }
    ...
    fun refuse(path: String): String? {
        ...
        return "拒绝:`$path` 是 HSUCODE 自己的运行时数据目录。" +
        ...
        for (p in prefixes) {
            for (sub in LOCKED) {
                if (command.contains("$p/$sub")) {
                    return "拒绝:这条命令要动 `$p/$sub` —— 那是 HSUCODE 自己的运行时数据。" +
