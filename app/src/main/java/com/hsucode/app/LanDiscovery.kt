 ...
 *
 * 协议很小,就是 UDP 广播一问一答:
 *   探测方 → 广播 {"type":"hsucode.probe"}
 *   在线方 → 单播回 {"type":"hsucode.announce", 设备名/型号/版本/端口...}
 *
 * 几个必须这么做的点:
...
object LanDiscovery {
    ...
    private const val TAG = "HsucodeLan"

    /** 固定端口。挑一个不常用的高位端口,避开常见服务。 */
    const val PORT = 47821

    private const val TYPE_PROBE = "hsucode.probe"
    private const val TYPE_ANNOUNCE = "hsucode.announce"
    ...
    fun startResponder(context: Context) {
        ...
        val t = Thread({
            ...
            try {
                ...
                lock = wifi?.createMulticastLock("hsucode-lan")?.apply {
        ...
        }, "hsucode-lan-responder").apply { isDaemon = true }
        responderThread = t
        t.start()
    }
    ...
    suspend fun discover(context: Context, timeoutMs: Long = 2500): List<Peer> =
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                lock = wifi?.createMulticastLock("hsucode-lan-probe")?.apply {
                    setReferenceCounted(false)
                    acquire()
                }
