 ...
 * `SQLiteCantOpenDatabaseException ... hsucode.db is not readable`,
...
class SelfProtectTest {
    ...
    private val pkgDir = "/data/user/0/com.hsucode.app"
    ...
    fun databasesDirIsProtected() {
        ...
        assertTrue(SelfProtect.isProtected("$pkgDir/databases/hsucode.db"))
        assertTrue(SelfProtect.isProtected("$pkgDir/databases/hsucode.db-wal"))
        assertTrue(SelfProtect.isProtected("$pkgDir/shared_prefs/x.xml"))
        assertNotNull(SelfProtect.refuse("$pkgDir/databases/hsucode.db"))
    }

    @Test
    fun traversalCannotSneakIn() {
        // files/../databases 规范化之后还是 databases。
        assertTrue(SelfProtect.isProtected("$pkgDir/files/../databases/hsucode.db"))
    }
    ...
    fun workspaceIsNeverProtected() {
        assertFalse(SelfProtect.isProtected("/storage/emulated/0/HSUCODE/a.md"))
        assertNull(SelfProtect.refuse("/storage/emulated/0/HSUCODE/rooms/x/plan.md"))
    }

    @Test
    fun shellCommandTouchingDatabasesIsRefused() {
        // 事故里就是这类命令。两种私有目录写法都要认。
        assertNotNull(SelfProtect.refuseCommand("chmod 000 $pkgDir/databases/hsucode.db"))
        assertNotNull(SelfProtect.refuseCommand("su -c 'chown root /data/data/com.hsucode.app/databases/hsucode.db'"))
        assertNotNull(SelfProtect.refuseCommand("rm -rf /data/user/0/com.hsucode.app/shared_prefs"))
    }

    @Test
    fun ordinaryShellCommandsPassThrough() {
        assertNull(SelfProtect.refuseCommand("ls -la /storage/emulated/0/HSUCODE"))
    ...
    fun guardIsInertUntilAppDirIsInjected() {
        ...
        assertFalse(SelfProtect.isProtected("$pkgDir/databases/hsucode.db"))
        assertNull(SelfProtect.refuseCommand("chmod 000 $pkgDir/databases/hsucode.db"))
    }
}
