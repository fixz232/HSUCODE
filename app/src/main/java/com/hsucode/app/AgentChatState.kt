...
import com.hsucode.data.MemoryExtractor
import com.hsucode.data.MessageEntity
import com.hsucode.provider.EmbeddingService
import com.hsucode.security.Decision
import com.hsucode.security.GateCommand
import com.hsucode.security.ToolConfirmResult
import com.hsucode.core.ToolBlockAction
...
class AgentChatState(
    ...
    private val openAiClient: com.hsucode.provider.OpenAiClient,
    /** 上下文压缩专用 client(功能模型配置 key="compact");未单独配置时行为与 openAiClient 一致。 */
    private val compactClient: com.hsucode.provider.OpenAiClient = openAiClient
) : ChatStateLike {
    companion object {
        private const val TAG = "AgentChatState"
        private val logFile by lazy { java.io.File("/data/data/com.hsucode.app/files/hsucode_memory.log") }
    ...
    private val scope = CoroutineScope(
        SupervisorJob() + Dispatchers.Main +
            com.hsucode.tools.WorkspaceThreadElement { sessionWorkspaceRoot to sessionProjectId }
    )
    ...
    /** Current session ID — set by HsucodeApplication on switch/create. */
    ...
    val confirmHandler: suspend (GateCommand, String) -> ToolConfirmResult = { cmd, preview ->
        ...
        pendingConfirm.value = ConfirmRequest(
            ...
            preview = preview,
            command = command,
            isIrreversible = cmd.reversibility == com.hsucode.security.Reversibility.IRREVERSIBLE
        )
        try {
            deferred.await()
        } finally {
