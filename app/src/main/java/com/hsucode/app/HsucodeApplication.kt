...
import com.hsucode.core.PowerMode
...
import com.hsucode.data.ProjectsRepository
import com.hsucode.data.IdentityEntity
import com.hsucode.data.IdentityRepository
...
import com.hsucode.security.SecurityGateImpl
import com.hsucode.tools.FileReadTool
import com.hsucode.tools.FileWriteTool
import com.hsucode.tools.ListDirTool
import com.hsucode.tools.DeleteFileTool
import com.hsucode.tools.MakeDirectoryTool
import com.hsucode.tools.DownloadFileTool
import com.hsucode.tools.SleepTool
import com.hsucode.tools.EditTool
import com.hsucode.tools.MultiEditTool
import com.hsucode.tools.GrepTool
import com.hsucode.tools.GlobTool
import com.hsucode.tools.ShellExecTool
import com.hsucode.tools.SuExecTool
import com.hsucode.tools.WebSearchTool
import com.hsucode.tools.WebFetchTool
    ...
    private fun installCrashGuard() {
        ...
        Thread.setDefaultUncaughtExceptionHandler { thread, throwable ->
            ...
            try {
                ...
                Log.e("HSUCODE", "UNCAUGHT on ${thread.name}", throwable)
...
override fun onCreate() {
        ...
        applicationScope.launch(Dispatchers.IO) {
            ...
            if (!ok) {
                ...
                Log.e("HsucodeApp", "ROOT INIT FAILED: ${RootShellManager.rootStatus}")
            } else {
                Log.i("HsucodeApp", "Root shell verified: uid=0 confirmed via libsu")
            }
        }
...
val suExecTool = SuExecTool().also { this.suExecTool = it }
        ...
        applicationScope.launch(Dispatchers.IO) {
            try { SkillImporter.autoDiscover(database) } catch (e: Exception) {
                Log.w("HsucodeApp", "skill auto-discover failed: ${e.message}")
            }
        }
        ...
        applicationScope.launch(Dispatchers.IO) {
            try {
                ...
                for (r in results) {
                    when (r) {
                        is McpConnectResult.Success -> Log.i("HSUCODE", "MCP restored: ${r.serverName} (${r.toolCount} tools)")
                        is McpConnectResult.Error -> Log.w("HSUCODE", "MCP restore failed: ${r.message}")
                    }
                }
            } catch (e: Exception) {
                Log.w("HSUCODE", "MCP syncFromRoom failed: ${e.message}")
            }
        }
        ...
        applicationScope.launch(Dispatchers.Main) {
            batteryMonitor.powerMode.collectLatest { mode ->
                ...
                Log.i("HSUCODE", "Power mode: ${mode.label}")
                ...
                Log.i("HSUCODE", "Wolfpack concurrency set to ${mode.wolfpackConcurrency} ($mode)")
            }
    }
    ...
    private fun buildSessionAgents(sessionId: Long): SessionAgents {
        ...
        holder.collector = applicationScope.launch(Dispatchers.Main) {
    ...
    private fun onCoreState(holder: SessionAgents, state: com.hsucode.core.AgentState) {
        ...
        if (sid == currentSessionId) {
            ...
            if (state is com.hsucode.core.AgentState.Idle && workflowState.toJson() != "[]") {
                val snapshot = workflowState.toJson()
                applicationScope.launch(Dispatchers.IO) {
                    database.trajectoryDao().upsert(
                        com.hsucode.data.TrajectoryEntity(sessionId = sid, eventsJson = snapshot)
                    )
                }
                workflowState.clear()
            }
    ...
    private fun refreshForegroundService() {
        ...
        if (text != null) {
            com.hsucode.service.AgentForegroundService.start(this, text)
        } else {
            com.hsucode.service.AgentForegroundService.stop(this)
        }
        com.hsucode.app.HsucodeWidgetReceiver.updateAll(this, text ?: "空闲")
    }

    private fun statusTextFor(state: com.hsucode.core.AgentState): String? = when (state) {
        is com.hsucode.core.AgentState.Idle -> null
        is com.hsucode.core.AgentState.Thinking -> "思考中… (第${state.iteration}轮)"
        is com.hsucode.core.AgentState.CallingTool -> "调用工具: ${state.toolName}"
        is com.hsucode.core.AgentState.Executing -> "执行: ${state.toolName}"
        is com.hsucode.core.AgentState.WaitingConfirm -> "等待确认: ${state.toolName}"
        is com.hsucode.core.AgentState.Responding -> "完成"
        is com.hsucode.core.AgentState.Error -> "✗ ${state.message}"
        is com.hsucode.core.AgentState.Interrupted -> "已中断"
    }
    ...
    private suspend fun seedOpeningStatement(sessionId: Long) {
        try {
            ...
            database.messageDao().insert(
                com.hsucode.data.MessageEntity(role = "assistant", content = opening, sessionId = sessionId)
            )
     ...
     */
    private fun buildGoalJudge(): com.hsucode.provider.JudgeService = runBlocking {
        ...
        com.hsucode.provider.JudgeService(
            baseUrl = cfg?.baseUrl ?: "",
            apiKey = key,
    ...
    fun switchToSession(id: Long) {
        ...
        if (existing != null) {
            ...
            _active = existing
            applicationScope.launch(Dispatchers.IO) { applyWorkspaceForSession(id) }
            refreshForegroundService()
            return
        }
    ...
    suspend fun applyWorkspaceForSession(sessionId: Long) {
        try {
            ...
            Log.w("HsucodeApp", "applyWorkspaceForSession failed: ${e.message}")
        }
    }
    ...
    fun setWebSearchEnabled(enabled: Boolean) {
        com.hsucode.tools.WebSearchGate.enabled = enabled
        toolRegistry.invalidateAvailability()
        applicationScope.launch(Dispatchers.IO) { database.settingDao().put("web_search_enabled", enabled.toString()) }
    }
    ...
    fun setCollabMode(enabled: Boolean) {
        ...
        com.hsucode.tools.CollabGate.enabled = enabled
        if (!enabled) {
    ...
    fun compactContext() {
        applicationScope.launch(Dispatchers.Main) {
            ...
            if (!ok) android.util.Log.w("HSUCODE", "compactContext: no-op or failed")
        }
    }
