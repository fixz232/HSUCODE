...
@Database(entities = [SettingEntity::class, MessageEntity::class, ProviderConfigEntity::class, SessionEntity::class, StateCursorEntity::class, AuditLogEntity::class, MemoryEntity::class, TrajectoryEntity::class, SkillEntity::class, McpServerEntity::class, GlobalSettingsEntity::class, ProjectEntity::class, IdentityEntity::class, PermissionRuleEntity::class, HookEntity::class, CronJobEntity::class, SubAgentEntity::class, UsageRecordEntity::class, KanbanTaskEntity::class, GroupRoomEntity::class, GroupMemberEntity::class, GroupMessageEntity::class, GroupRoomSummaryEntity::class, KanbanRunEntity::class, CodeSymbolEntity::class, CodeEdgeEntity::class, CodeFileEntity::class, CommandRoomRunEntity::class, CommandRoomEventEntity::class], version = 44, exportSchema = true)
...
abstract class AppDatabase : RoomDatabase() {
    ...
    abstract fun commandRoomHistoryDao(): CommandRoomHistoryDao

    companion object {
        @Volatile
        private var INSTANCE: AppDatabase? = null

        const val DB_NAME = "hsucode.db"
        ...
        private val MIGRATION_18_19 = object : Migration(18, 19) {
            override fun migrate(db: SupportSQLiteDatabase) {
                ...
                db.execSQL("""
                    ...
                    VALUES (
                        '默认助手',
                        '你是 HSUCODE,一个跑在 Android root 设备上的 AI Agent,可调用工具完成任务。回答简洁、技术化、不啰嗦。',
                        1.0,
                        $now
                    )
                """.trimIndent())
        ...
        private val MIGRATION_40_41 = object : Migration(40, 41) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("""
                    CREATE TABLE IF NOT EXISTS command_room_runs (
                ...
                """.trimIndent())
                db.execSQL("CREATE INDEX IF NOT EXISTS index_command_room_runs_startedAt ON command_room_runs(startedAt)")
                db.execSQL("""
                    CREATE TABLE IF NOT EXISTS command_room_events (
                        id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
                        runId TEXT NOT NULL,
                ...
                """.trimIndent())
                db.execSQL("CREATE INDEX IF NOT EXISTS index_command_room_events_runId ON command_room_events(runId)")
                db.execSQL("CREATE INDEX IF NOT EXISTS index_command_room_events_workerRunId ON command_room_events(workerRunId)")
            }
        }
        ...
        /** 子智能体中心:显示身份标识、允许停用,并让每个角色使用独立采样温度。 */
        private val MIGRATION_42_43 = object : Migration(42, 43) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE sub_agents ADD COLUMN avatar TEXT NOT NULL DEFAULT 'agent'")
                db.execSQL("ALTER TABLE sub_agents ADD COLUMN enabled INTEGER NOT NULL DEFAULT 1")
                db.execSQL("ALTER TABLE sub_agents ADD COLUMN temperature REAL NOT NULL DEFAULT 0.7")
                db.execSQL("""
                    UPDATE sub_agents SET avatar = CASE name
                        WHEN '探索者' THEN 'search'
                        WHEN '审查员' THEN 'review'
                        WHEN '编码员' THEN 'code'
                        WHEN '研究员' THEN 'search'
                        ELSE avatar
                    END
                    WHERE builtin = 1
                """.trimIndent())
        ...
        /**
         * Upstream v1.20 group-room and model-state upgrade, renumbered after
         * HSUCODE's own 40-43 migrations so existing installations keep their data.
         */
         ...
         *
         * 为什么必须有这一层:
         * - 禁止 Room 的破坏性迁移兜底:找不到迁移路径时也必须进入这里,先完整保留旧库。
         *   迁移跑完之后 Room 还会拿实体声明和数据库实况逐项比对(列、类型、主键、索引……),
         *   这一步不匹配同样会抛 IllegalStateException。
        ...
        private fun openOrRecover(context: Context): AppDatabase {
            ...
            try {
                ...
                // 结果是 "Permission denied ... hsucode.db is not readable"。注意这时
                ...
                if (tryRepairPermissions(context)) {
                    ...
                    if (retried != null) {
                        android.util.Log.w("HsucodeDb", "数据库权限已修复,原样打开(未重建): $reason")
                        return retried
                    }
                }

                android.util.Log.e("HsucodeDb", "打开数据库失败,改名备份后重建: $reason", t)
        ...
        private fun tryRepairPermissions(context: Context): Boolean {
            ...
            if (changed) android.util.Log.w("HsucodeDb", "尝试修复数据库文件权限")
            return changed
        }
