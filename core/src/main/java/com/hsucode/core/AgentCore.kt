...
import com.hsucode.data.StateCursorDao
import com.hsucode.data.StateCursorEntity
import com.hsucode.provider.AgentStreamResult
import com.hsucode.provider.ApiError
import com.hsucode.provider.OpenAiClient
import com.hsucode.security.*
...
/**
 * Heart of HSUCODE: the Agent while-loop.
 *
...
class AgentCore(
     ...
     * (HSUCODE 每轮工具结果都在同一迭代内回灌,迭代顶部注入 user 消息是安全的)。
     */
     ...
     */
    fun restoreHistory(entities: List<com.hsucode.data.MessageEntity>) {
    ...
    private fun launchLoop(scope: CoroutineScope, block: suspend () -> Unit): Job {
        currentJob = scope.launch {
            try {
                ...
                Log.d("HSUCODE", "state -> Idle, agentCore.isBusy=${_state.value.isBusy}")
            }
        }
        return currentJob!!
    }
