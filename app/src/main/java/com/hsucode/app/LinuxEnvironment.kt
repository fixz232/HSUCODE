...
object LinuxEnvironment {
    ...
    fun init(context: Context) {
        ...
        rootfsDir = dir
        state = if (File(dir, ".hsucode_ready").exists()) State.READY else State.NOT_SETUP
    }

    fun isReady(): Boolean = state == State.READY && rootfsDir?.let { File(it, ".hsucode_ready").exists() } == true
    ...
    suspend fun bootstrap(context: Context, rootfsUrls: List<String> = ROOTFS_URLS, force: Boolean = false): Boolean = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            // 用 App 可读的 .hsucode_ready 标记判断已部署(bin/bash 属 root,App 进程 stat 不到,不能用)。
            if (!force && File(dir, ".hsucode_ready").exists()) {
            ...
            File(dir, ".hsucode_ready").writeText("1")
            state = State.READY
            log("Linux 环境就绪 ✓")
