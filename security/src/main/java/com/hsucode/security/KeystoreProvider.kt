 ...
 */
class KeystoreProvider(private val alias: String = "hsucode_master_key") {

    companion object {
        private const val ENVELOPE_PREFIX = "hsucode:v1:"
    }
     ...
     * 并发的 cipher.init/doFinal 会抛 "Keystore operation failed"。HSUCODE 里 wolfpack_run 并行拉起多个
    ...
    /** Store an encrypted value in text-only persistence fields. */
    fun encryptString(plaintext: String): String {
        if (plaintext.isBlank()) return ""
        val encoded = android.util.Base64.encodeToString(encrypt(plaintext), android.util.Base64.NO_WRAP)
        return ENVELOPE_PREFIX + encoded
    }

    /** Decrypts new envelopes while allowing existing plaintext rows to migrate in place. */
    fun decryptString(stored: String): String {
        if (stored.isBlank() || !stored.startsWith(ENVELOPE_PREFIX)) return stored
        val encoded = stored.removePrefix(ENVELOPE_PREFIX)
        return decrypt(android.util.Base64.decode(encoded, android.util.Base64.NO_WRAP))
    }

    fun isEncryptedString(value: String): Boolean = value.startsWith(ENVELOPE_PREFIX)
}
