...
class SecurityGateImpl(
    private val auditLogDao: com.hsucode.data.AuditLogDao? = null,
    private val auditScope: CoroutineScope = CoroutineScope(SupervisorJob() + Dispatchers.IO)
) : SecurityGate {
    ...
    @Volatile
    private var permissionRules: List<com.hsucode.data.PermissionRuleEntity> = emptyList()
    override fun setPermissionRules(rules: List<com.hsucode.data.PermissionRuleEntity>) {
    ...
    companion object {
        ...
        val SAFE_COMMANDS = setOf(
            ...
            "ps", "printenv", "which", "type", "basename", "dirname", "realpath",
    ...
    private fun isSafeReadOnlyCommand(raw: String): Boolean {
        ...
        // A command classified as read-only is still evaluated by sh -c. Reject every construct
        // that can run hidden commands or mutate state before applying the command whitelist.
        ...
        for (segRaw in segments) {
            ...
            if (head == "git") {
                ...
                if (tokens.any { it in setOf("-exec", "-execdir", "-ok", "-okdir", "-delete", "-fprint", "-fprintf") }) return false
    ...
    override fun audit(cmd: GateCommand, decision: Decision, result: String?) {
        ...
        val entry = com.hsucode.data.AuditLogEntity(
            timestamp = System.currentTimeMillis(),
            toolName = cmd.toolName,
            toolArgs = redactSecrets(cmd.toolArgs),
            capability = cmd.capability.name,
            reversibility = cmd.reversibility.name,
            decision = ds,
            result = result?.let(::redactSecrets)
        )
        if (auditLogDao != null) {
            auditScope.launch { runCatching { auditLogDao.insert(entry) } }
        } else {
            synchronized(auditTrail) {
                auditTrail.add(AuditEntry(System.currentTimeMillis(), cmd.toolName, entry.toolArgs,
                    cmd.capability, cmd.reversibility, ds, entry.result))
            }
        }
    }
    ...
    private fun redactSecrets(raw: String): String {
        ...
        val redacted = if (parsed != null) redactJsonObject(parsed).toString() else raw
        return redacted
            .replace(Regex("(?i)(bearer\\s+)[A-Za-z0-9._~+/=-]+"), "$1[REDACTED]")
            .replace(
                Regex("(?i)(api[_-]?key|access[_-]?token|password|secret|authorization|cookie)(\\s*[:=]\\s*)[^\\s,;]+"),
                "$1$2[REDACTED]"
            )
    }

    private fun redactJsonObject(source: JSONObject): JSONObject = JSONObject().apply {
        val keys = source.keys()
        while (keys.hasNext()) {
            val key = keys.next()
            val value = source.opt(key)
            val sensitive = key.contains(Regex("(?i)(token|key|secret|password|authorization|cookie)"))
            put(key, when {
                sensitive -> "[REDACTED]"
                value is JSONObject -> redactJsonObject(value)
                else -> value
            })
        }
    }
